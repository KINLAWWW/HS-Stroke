...
class ArjunViT_R(nn.Module):
    def __init__(
        ...
        assert chunk_size % t_patch_size == 0, (
            f"EEG chunk size {chunk_size} must be divisible by temporal patch size {t_patch_size}."
        )
        assert pool_func in {"cls", "mean"}, (
            "pool_func must be either cls or mean"
        )
        ...
        self.dropout_layer = nn.Dropout(embed_dropout)
        ...
        self.transformer = Transformer(
            hid_channels, depth, heads, head_channels, mlp_channels, dropout
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x[:, 0] if self.pool_func == "cls" else x.mean(dim=1)
