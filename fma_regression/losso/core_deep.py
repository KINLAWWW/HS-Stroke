"""LOSSO deep-learning runner for FMA regression."""
...
from hs_stroke.splits import LeaveOneSubjectSessionOut
...
def _infer_session_id_from_trial_id(trial_id: str) -> str:
    trial_id = str(trial_id)
    matched = re.search(r'(ses[-_]?\d+)', trial_id)
    if matched is not None:
        return matched.group(1).replace('_', '-')
    normalized = os.path.basename(os.path.dirname(trial_id))
    if normalized:
        return normalized
    raise ValueError(f'Cannot infer session_id from trial_id: {trial_id}')
...
def _ensure_split_columns(dataset: FMADataset) -> None:
    ...
    if 'session_id' not in dataset.info.columns:
        if 'trial_id' not in dataset.info.columns:
            raise KeyError('dataset.info does not contain session_id or trial_id.')
        dataset.info['session_id'] = dataset.info['trial_id'].apply(_infer_session_id_from_trial_id)
        print('[info] session_id column was inferred from trial_id for cached data.', flush=True)
...
def run_experiment(
    *,
    ...
    _ensure_split_columns(dataset)

    print(f'[info] dataset size: {len(dataset)}', flush=True)
    print('[info] LOSSO uses one (subject_id, session_id) as test and keeps the same subject\'s other session in training.', flush=True)
    ...
    for model_name in models:
        ...
        data_split = LeaveOneSubjectSessionOut(split_path=model_split_path)

        rows = []
        subject_session_count = 0
        for fold_idx, (train_set, test_set, subject_session) in enumerate(data_split.split(dataset)):
            subject_session_count += 1
            if max_subject_sessions > 0 and subject_session_count > max_subject_sessions:
                break
            ...
            test_subject = str(test_set.info['subject_id'].iloc[0])
            test_session = str(test_set.info['session_id'].iloc[0])
            same_subject_train_sessions = sorted(train_set.info.loc[train_set.info['subject_id'] == test_subject, 'session_id'].astype(str).unique().tolist())

            print(
                f'[train] model={model_name} subject_session={subject_session} fold={fold_idx + 1} '
                f'test_subject={test_subject} test_session={test_session} '
                f'train={len(train_set)} val={len(val_set)} test={len(test_set)} '
                f'same_subject_train_sessions={same_subject_train_sessions} params={count_parameters(model)}',
                flush=True,
            )
            ...
            rows.append({
                ...
                'same_subject_train_sessions': '|'.join(same_subject_train_sessions),
            })
