"""Session-level Ridge baseline runner for FMA regression under LOSSO."""
...
def run_experiment(
    *,
    ...
    results_dir=RESULTS_ROOT / "fma_regression" / "losso" / "sbp_ridge",
    alpha_grid=(0.1, 1.0, 10.0),
):
    _log('[step] start LOSSO Ridge baseline')
    ...
    rows: List[Dict] = []
    all_test_indices = list(range(len(samples)))
    _log(f'[split] total LOSSO folds={len(all_test_indices)} (one subject-session held out each fold)')

    for fold_no, test_idx in enumerate(all_test_indices, start=1):
        test_subject = subjects[test_idx]
        te_idx = np.asarray([test_idx])
        tr_idx = np.asarray([idx for idx in all_test_indices if idx != test_idx])
        ...
        _log(
            f'[split {fold_no:02d}/{len(all_test_indices)}] '
        ...
        _log(f'[split {fold_no:02d}/{len(all_test_indices)}] selected_alpha={best_alpha}')
